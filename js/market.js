...
(async function loadMarket() {
  ...
  const MARKET_URL = "https://brilique-ai-jewelry-backend-4.onrender.com/api/designs/market";
  const USERS_URL  = "https://brilique-ai-jewelry-backend-4.onrender.com/api/user/all";
  const DEFAULT_AVATAR = "images/微信图片_20211105222712.png";

  try {
    // Fetch in parallel
    const [resDesigns, resUsers] = await Promise.all([fetch(MARKET_URL), fetch(USERS_URL)]);

    const [designText, usersText] = await Promise.all([resDesigns.text(), resUsers.text()]);
    if (!resDesigns.ok) throw new Error(designText || `HTTP ${resDesigns.status}`);

    let designs = [];
    let users = [];
    try { designs = JSON.parse(designText); } catch {}
    try { users   = resUsers.ok ? JSON.parse(usersText) : []; } catch {}
    ...
    designs.forEach(d => {
      const imageUrl = d.imageUrl || "";
      const emailKey = String(d.userId || "").toLowerCase(); // userId holds email
      const match = userByEmail.get(emailKey);

      const displayName = match?.name || d.userId || "Unknown";
      const avatarUrl   = match?.profilePicUrl || DEFAULT_AVATAR;
